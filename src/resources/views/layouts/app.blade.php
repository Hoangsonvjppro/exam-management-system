...
<body class="font-sans antialiased bg-gray-50 text-gray-900">
    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
        ...
        {{-- ─── SIDEBAR ─────────────────────────────────────────── --}}
        <!-- Overlay (mobile) -->
        <div class="fixed inset-0 z-20 bg-black/50 lg:hidden" x-show="sidebarOpen" x-on:click="sidebarOpen = false"
        ...
        <!-- Sidebar -->
        <aside
            class="fixed inset-y-0 left-0 z-30 w-64 flex flex-col bg-indigo-900 text-white transition-transform duration-300 lg:static lg:translate-x-0"
            :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" style="transform: translateX(-100%);">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
                            ...
                            <div class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50"
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    ...
                                    <button type="submit"
                                        ...
                                        <svg class="w-4 h-4 mr-2.5" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-6">
                ...
                @if (session('error'))
                    <div
                        class="mb-4 px-4 py-3 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm flex items-center gap-2">
                        <svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
