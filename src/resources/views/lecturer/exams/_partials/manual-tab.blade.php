...
<div id="panel-manual" class="flex-1 flex flex-col">
    <div class="bg-white rounded-[10px] border-[0.5px] border-[#D6E2F0] flex flex-col h-full shadow-sm">
        ...
        <div class="p-5 border-b border-[#EBF2FA] bg-[#F8FAFD] rounded-t-[10px]">
            <div class="flex justify-between items-center mb-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" class="btn btn-ghost btn-sm" data-action="open-quick-question-modal">+ Thêm câu hỏi</button>
            ...
            <div class="filter-bar">
                <input type="text" id="manual-search" class="ca-input search-input" placeholder="🔍 Tìm kiếm theo từ khóa..." data-action="manual-search-input">
                <select id="manual-chapter-filter" class="ca-select" data-action="manual-filter-change" style="min-width:150px;flex:1">
                    <option value="">Tất cả chương</option>
                </select>
                <select id="manual-difficulty-filter" class="ca-select" data-action="manual-filter-change" style="min-width:130px;flex:0.7">
                    <option value="">Tất cả độ khó</option>
                    @foreach($difficulties as $diff)
                    <option value="{{ $diff->code }}">{{ $diff->name }}</option>
                    @endforeach
                </select>
                <button type="button" class="btn btn-ghost btn-sm" id="collapse-all-btn" data-action="collapse-all-previews" style="display:none">
                    ↕ Thu gọn
                </button>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-5 py-4 max-h-[700px] min-h-[400px]" id="questions-container">
            ...
            {{-- Load More button --}}
            <button type="button" id="load-more-btn" class="load-more-btn" data-action="load-more-questions" style="display:none">
                Tải thêm câu hỏi...
            </button>
