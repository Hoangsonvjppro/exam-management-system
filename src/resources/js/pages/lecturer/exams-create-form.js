        ...
        div.innerHTML = `
            <div class="flex items-start gap-3">
                <input type="checkbox" class="mt-1 rounded border-[#D6E2F0] text-[#185FA5] focus:ring-[#E6F1FB] w-4 h-4 cursor-pointer flex-shrink-0"
                    ${isSelected ? 'checked' : ''} data-action="toggle-question" data-question-id="${q.id}">
                <div class="flex-1 min-w-0">
                    ...
                    <button type="button" class="text-[13px] text-[#374151] line-clamp-2 cursor-pointer text-left w-full" data-action="toggle-question-preview" data-question-id="${q.id}">${plainContent}${q.content.length > 200 ? '...' : ''}</button>
                </div>
                <button type="button" class="text-[#6B7C99] hover:text-[#1A3A6B] flex-shrink-0 p-1" data-action="toggle-question-preview" data-question-id="${q.id}" title="Xem chi tiết">
...
function addMatrixRow(prefill) {
    ...
    row.innerHTML = `
        <td><select name="matrix[${i}][chapter_id]" class="ca-select" style="min-width:140px" data-action="matrix-row-change"><option value="">Tất cả</option>${chapterOpts}</select></td>
        <td><select name="matrix[${i}][difficulty]" class="ca-select" required style="min-width:120px" data-action="matrix-row-change">${diffOpts}</select></td>
        <td><input type="number" name="matrix[${i}][question_count]" class="ca-input matrix-count" value="${prefill?.question_count ?? prefill?.count ?? 5}" min="1" required style="width:70px" data-action="matrix-row-input"></td>
        <td class="text-center"><span class="availability-hint" data-avail>—</span></td>
        <td class="text-center"><button type="button" data-action="remove-matrix-row" class="text-[#DC2626] hover:text-[#991B1B] text-[14px] font-bold">&times;</button></td>
    `;
...
function bindDomEvents() {
    document.querySelectorAll('[data-action="switch-creation-mode"]').forEach((button) => {
    ...
    document.querySelector('[data-action="open-quick-question-modal"]')?.addEventListener('click', () => {
    ...
    document.querySelectorAll('[data-action="manual-filter-change"]').forEach((element) => {
        element.addEventListener('change', () => {
            searchQuestions(1);
        });
    });

    document.querySelector('[data-action="collapse-all-previews"]')?.addEventListener('click', () => {
        collapseAllPreviews();
    });

    document.querySelector('[data-action="load-more-questions"]')?.addEventListener('click', () => {
        loadMoreQuestions();
    });

    document.getElementById('questions-list')?.addEventListener('change', (event) => {
        const checkbox = event.target.closest('[data-action="toggle-question"]');
        if (!checkbox) {
            return;
        }

        const questionId = Number(checkbox.dataset.questionId || 0);
        if (!questionId) {
            return;
        }

        toggleQuestion(questionId, checkbox.checked);
    });

    document.getElementById('questions-list')?.addEventListener('click', (event) => {
        const trigger = event.target.closest('[data-action="toggle-question-preview"]');
        if (!trigger) {
            return;
        }
        ...
        togglePreview(questionId);
    });

    document.getElementById('preset-total')?.addEventListener('input', () => {
        updateActivePreset();
    });
    ...
    document.querySelector('[data-action="clear-matrix"]')?.addEventListener('click', () => {
        clearMatrix();
    });

    document.querySelector('[data-action="add-matrix-row"]')?.addEventListener('click', () => {
        addMatrixRow();
    });

    document.getElementById('matrix-body')?.addEventListener('change', (event) => {
        const input = event.target.closest('[data-action="matrix-row-change"]');
        if (!input) {
            return;
        }
    ...
    document.getElementById('matrix-body')?.addEventListener('input', (event) => {
        const input = event.target.closest('[data-action="matrix-row-input"]');
        if (!input) {
            return;
        }
    ...
    document.getElementById('matrix-body')?.addEventListener('click', (event) => {
        const removeButton = event.target.closest('[data-action="remove-matrix-row"]');
        if (!removeButton) {
            return;
        }
    ...
    document.querySelector('[data-action="add-quick-question-option"]')?.addEventListener('click', () => {
        addQuickQuestionOption();
    });

    document.getElementById('quick-question-options')?.addEventListener('click', (event) => {
        const removeButton = event.target.closest('[data-action="remove-quick-question-option"]');
        if (!removeButton) {
            return;
        }

        removeQuickQuestionOption(removeButton);
    });

    document.getElementById('quick-question-form')?.addEventListener('submit', (event) => {
        submitQuickQuestionForm(event);
    });
...
Object.assign(window, {
    ...
    checkMatrixAvailability,
    addQuickQuestionOption,
    removeQuickQuestionOption,
    submitQuickQuestionForm,
    openQuickQuestionModal,
    toggleLateSettings,
});
