    ...
    @else
        <div class="w-[48px] flex items-center justify-center flex-shrink-0">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $icons[$icon ?? 'circle'] ?? $icons['circle'] }}"/>
            </svg>
        </div>
    @endif
    <span class="sidebar-label truncate flex-1 transition-opacity duration-300 pr-4" x-show="isExpanded" x-cloak>{{ $slot }}</span>
    @if($badge)
        <span class="ml-auto mr-4 bg-danger-500 text-white text-xs font-bold px-2 py-0.5 rounded-full" x-show="isExpanded" x-cloak>{{ $badge }}</span>
    @endif
</a>
