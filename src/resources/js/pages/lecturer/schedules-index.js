...
const scheduleIndexConfigEl = document.getElementById('schedule-index-config');
const scheduleIndexConfig = scheduleIndexConfigEl ? JSON.parse(scheduleIndexConfigEl.textContent || '{}') : {};

window.scheduleRowMenuState = function scheduleRowMenuState() {
    return {
        open: false,
...
window.scheduleManager = function scheduleManager() {
    return {
        isSubmitting: false,
        selectedExamId: '',
        selectedSubjectId: '',
        ...
        quickPreviewUrlTemplate: scheduleIndexConfig.quickPreviewUrlTemplate || '',
        quickUpdateUrlTemplate: scheduleIndexConfig.quickUpdateUrlTemplate || '',
        examEditUrlTemplate: scheduleIndexConfig.examEditUrlTemplate || '',
        csrfToken: scheduleIndexConfig.csrfToken || document.querySelector('meta[name="csrf-token"]')?.content || '',
        ...
        async submitCreateForm(formElement) {
            ...
            try {
                const response = await fetch(scheduleIndexConfig.scheduleStoreUrl || '', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json',
        ...
        async submitQuickExamForm(formElement) {
            ...
            try {
                const response = await fetch(scheduleIndexConfig.examStoreUrl || '', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json',
